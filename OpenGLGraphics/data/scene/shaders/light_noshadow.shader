...
void main() 
{
	...
	//gl_FragColor = (dot(light_dir,normal) * base + pow(clamp(dot(reflect(-light_dir,normal),camera_dir),0.0,1.0),16.0) * gloss.w) * gl_TexCoord[3];
	gl_FragColor = dot(light_dir, normal) * base * gl_TexCoord[3];
